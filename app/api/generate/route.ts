...
export async function POST(request: NextRequest) {
  try {
    ...
    const { text } = await generateText({
      model: openai("gpt-4o"),
      prompt,
      system: `You are an expert marketing content creator specializing in GEO (Generative Engine Optimization).
               Create engaging, high-quality content optimized for AI-powered search engines and generative AI systems.
               Focus on clear context, structured information, entity recognition, and direct answers that AI can easily parse and cite.
               Prioritize factual accuracy, clear attribution, and comprehensive coverage of topics.`,
    })

    // Calculate GEO score (simplified scoring algorithm)
    const geoScore = calculateGEOScore(text, targetKeywords || "")
...
function buildPrompt(params: any) {
  ...
  prompt += ` Ensure the content is GEO-optimized for AI systems, engaging, and provides clear, factual value with proper context and structure.`

  return prompt
}

function calculateGEOScore(content: string, keywords: string): number {
